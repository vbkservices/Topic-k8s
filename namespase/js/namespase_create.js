...
$(document).ready(function () {
    ...
    <div class="col-sm-4 pt-2">
        <input class="mx-2" type="radio" name="sex" onclick="imagesadd('defaults')" value="yes">defaults
        <input class="mx-2" type="radio" name="sex" onclick="imagesadd('`+ name + `')" value="no">` + name + `<p>
    </div>
...
function images_dropdown_status() {
    ...
    function showValue(e) {
        ...
        <div class="toast-header">
          <strong class="me-auto">`+values+`</strong>
...
function add_students() {
    $.ajax({ //kubectll get pods
        type: "post",
        url: 'http://120.114.142.17/sys/namespase/prg/kube_add_student.php',
        // dataType: "json",
        data: fddyaml,
        processData: false,
...
function new_namespase() {
    ...
    let str="";
    $(".toast-header").each(function(){
        var textValues = $(this).children('strong').map(function() {
            return $(this).text();
          }).get();
    ...
    $.ajax({ //kubectll get pods
        ...
        success: function (result, status) {
            document.location.href = "http://120.114.142.17/sys/namespase/namespase.html";
        }
...
function new_container() {
    ...
    $.ajax({ //kubectll get pods
        type: "post",
        url: 'http://120.114.142.17/sys/namespase/prg/kube_container_create.php',
        // dataType: "json",
        data: fddyaml,
        processData: false,
        //將原本不是xml時會自動將所發送的data轉成字串(String)的功能關掉
