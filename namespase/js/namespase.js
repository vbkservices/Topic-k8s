        ...
        success: function (result, status) {
            ...
            Object.entries(result).forEach(([rekeys, revalues]) => {
                if(revalues[3]=="show"){
                    ...
                    image=`<b class="text-dark">`+revalues[1]+`:`+revalues[2]+`<b/>`
                    ...
                    image=`<b class="text-secondary">`+revalues[1]+`:`+revalues[2]+`<b/>`
                }
...
function nsdelete(namepase,status) {
   /* document.querySelector('#images_delete').innerHTML = "<div>刪除中</div>";*/
    fddjs.append('type', "namespace");
    fddjs.append('namepase', namepase);
    fddjs.append('status', status);
    $.ajax({ //kubectll get pods
...
function status_pod(namepase) {
    ...
    window.location = "http://dic-con.vbfaka.com/sys/namespase/namespase_container.html"
}