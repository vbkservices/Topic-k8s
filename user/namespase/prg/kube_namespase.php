...
if($enapikey == "2727175#356"){
    ...
    if($status=="namespace"){
        ...
        if($k8s_user[0]["remark"]=="teacher"  ){
            foreach($output["items"] as $value) {
                if($value["metadata"]["labels"]["user"]!=null && $value["metadata"]["labels"]["user"]==$name){
                    ...
                    $namespace_name = $database->select("namespace_name", [
                        "namespace"
                    ], [
                        "name" => ''.$value["metadata"]["name"].''
                    ]);
                    array_push($array,$value["metadata"]["name"],$data_user[0]["name"],$value["metadata"]["creationTimestamp"],$value["metadata"]["labels"]["status"],$namespace_name[0]["namespace"]);
                    array_push($array_json,$array);
                }
            }
        }else if($k8s_user[0]["remark"]=="student"){
            foreach($output["items"] as $value) {
                if($value["metadata"]["labels"]["user"]!=null && $value["metadata"]["labels"]["student.".$name.""]==$name){
                    $array=[];
                    $data_user = $database->select("k8s_userdb", [
                        "name"
                    ], [
        ...
        }else if($k8s_user[0]["remark"]=="root"){
            foreach($output["items"] as $value) {
        ...
        if($output["code"]!=404){
            ...
            foreach($output["items"] as $value) {
                if($value["metadata"]["labels"]["user"]!=null){
                    ...
                    array_push($array,$value["status"]["phase"],$value["spec"]["containers"][0]["image"],$data_user[0]["name"],$value["metadata"]["name"],$value["metadata"]["labels"]["conrainer-status"],$namespace_name[0]["namespace"]);
                    array_push($array_json,$array);
                }
            }
        }else{
            array_push($array_json,$namepase,"false");
        }
        echo json_encode($array_json);
    }
    if($status=="mypod"){
        ...
        $array_json=["".$namespace_name[0]["namespace"].""];
        foreach($output["items"] as $value) {
