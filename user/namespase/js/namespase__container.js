...
window.onload = function () {
    $.ajax({ //kubectll get pods
        ...
        success: function (result, status) {
            ...
            <button type="button" class="btn btn-success" onclick="new_model_container('`+pod+`','`+see_port+`')"><b>建立課程容器</b></button>
          </div>`;
...
function namespace_pod(see_port ) {
    ...
    function new_model_container(namepase,port) {
        $('#exampleModal').modal({ backdrop: 'static', keyboard: false });
        document.getElementById('model_button').innerHTML = `<button  type="button" class="btn btn-primary my-2 mx-4" onclick="new_container('` + namepase + `','` + port + `')"><b>建立</b>
        <button type="button" class="btn btn-primary my-2"data-bs-dismiss="modal"><b>取消</b></button>`;
        $('#exampleModal').modal('show');
        namepace_images_status(namepase,port)
    }
    ...
    function new_container(namepase,port) {
