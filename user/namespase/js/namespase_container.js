...
let see_port = '';
if(!namepase){
    window.location = "http://dic-con.vbfaka.com/sys/user/index.html"
}
window.onload = function () {
    let heght= document.querySelector('body').clientHeight*0.80;
    ...
    <ol class="breadcrumb m-0">
        <li class="breadcrumb-item px-3"><a
         href="http://dic-con.vbfaka.com/sys/user/namespase/namespase.html">
                ...
                if(result[0]){
                    if(result[0][0]=="Pending"){
                        ...
                        <div class="card-body text-start">
                          ...
                          <button class="btn btn-danger mx-2" onclick="close_container('`+result[0][3]+`')">刪除機器</button>
                          </h5></div></div>`;
                        $('#mypod').html(table);
                        ...
                        if(result[0][6]=="running"){
                            ...
                            let table=``;
                            table=table+`</p><div class="col-3 text-end"><b>課程名稱:</b></div><div class="col-9 text-start"><b>`+namepace_name+`</br><i class="fa fa-arrow-right mx-2"></i>`+result[0][1]+`</b></div></p>`;
                            ...
                            new_model_container(namepase,namepace_name);
                        }
        ...
        success: function (result, status) {
            ...
            if(result=="false"){
                ...
                Object.entries(result).forEach(([key, value]) => { 
                    ...
                    table = table + `<tr>
                      <th scope="row">`+ value[5] + `<i class="fa fa-arrow-right mx-2"></i>`+value[1]+`</th>
                      <td>`+ value[2] + `</td>
                      <td>`+ value[0] + `</td>
                      <td>`+revalue+`</td>
                    </tr>`;
...
function new_model_container(namepase,namepace_name) {
    console.log(namepace_name);
    let table=`<h4 class="text-center modal-title"><b>目前課程:`+namepace_name+`</b></h4>
    <div class="form-group row my-2" id="open_contaner">
        ...
        <div class="col-sm-5 py-2">
            ...
            <select class="form-control form-control-sm form-select" id="namepace_images_status">
    ...
    $('#mypod').html(table);
        ...
        $.ajax({ //kubectll get pods
            ...
            success: function (result, status) {
                    ...
                    //    console.log(revalue);
                        if(revalue[3]=="show"){
                            table = table + (`<option value="` + revalue[0] + `.` + revalue[1] + `:` + revalue[2] + `">` + revalue[4] + `</option>`);
                        }
                    });
    ...
    function open_see_teacher(name, user, pod_name, namepase, image, port, see_port) {
        ...
        $.ajax({ //kubectll get pods
            ...
            contentType: false,
            success: function (result, status) {
                let xtream = `<iframe frameborder="0" noresize="noresize" src="http://dic-con.vbfaka.com:` + see_port + `/?arg=` + result + `" frameborder="0" style=" width: 100%; height:100%;"></iframe>`
                $('#xtream').html(xtream);
            }
        });
    }*/
