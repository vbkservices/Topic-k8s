    ...
    success: function (result, status) {
        ...
        Object.entries(demo.items).forEach(([key, value]) => {
            if(value.metadata.namespace != "kube-system" && value.metadata.namespace != "kube-public" ){
                ...
                table = table +`<tr><th scope="row"> ${value.metadata.name}</th><td>`+label1+`</td>><td> ${value.spec.containers[0].image}</td>><td>`+GetDateDiff1+`</td><td> ${value.metadata.namespace}</td></tr>`;
            }
        });
